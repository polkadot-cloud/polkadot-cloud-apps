...
type OverviewEntries = [string, ValidatorOverview][]
const EMPTY_OVERVIEWS: ValidatorOverviews = new Map()
// Cache plain entries so React Query can preserve equal snapshots and memoize this lookup map.
const selectOverviews = (entries: OverviewEntries): ValidatorOverviews =>
	new Map(entries)
...
export const useValidatorOverviews = (
	...
	const result = useDataPoint<OverviewEntries, ValidatorOverviews>(
		{
			...
			node: {
				...
				queryFn: async ({ signal }) => {
					...
					return overviews.map(([[, address], overview]) => [address, overview])
				},
			},
			stakingApi: {
